...
echo "  2. ./deploy.sh openobserve       # Deploy shared + OpenObserve"
echo "  3. ./deploy.sh grafana-stack     # Deploy shared + Grafana stack"
echo ""
...
echo ""
echo "NOTE: .env files are written to the repo directory and picked up"
echo "on the first deploy. Subsequent deploys carry .env forward from"
echo "the previous release — re-running setup.sh will NOT automatically"
echo "update a running deployment. To force new values, either:"
echo "  - ENV_SOURCE=source ./deploy.sh <stack>   (use repo .env)"
echo "  - Edit .env directly in /opt/observability-deploy/current/<component>/"
