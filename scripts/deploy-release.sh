...
chmod a+r "$RELEASE_DIR"/prometheus/*.yml "$RELEASE_DIR"/prometheus/alerts/*.yml \
  ...
  "$RELEASE_DIR"/otel-collector/otel-collector-config.yml "$RELEASE_DIR"/caddy/Caddyfile 2>/dev/null || true
...
compose up -d grafana prometheus loki tempo alertmanager redis-autolog autolog-service promtail caddy
compose up -d otel-collector

echo "Waiting for service health..."
if ! wait_for_services "$HEALTH_TIMEOUT_SECONDS" \
  grafana prometheus loki tempo alertmanager redis-autolog caddy otel-collector; then
  echo "ERROR: deployment health check failed"
  compose ps
  compose logs --tail=120 tempo prometheus alertmanager caddy otel-collector || true
